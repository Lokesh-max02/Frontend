...
-- task 1
select department,count(*)"Total Employees",sum(salary)"Total salary",avg(salary)"Average salary",max(salary)"Highest salary",min(salary)"Lowest salary" from Employees group by department;
-- task 2
select city,count(*)"Total Employees",max(salary)"Highest salary",min(salary)"Lowest salary",avg(salary)"Average salary" from Employees  group  by city order by max(salary)DESC;
-- task 3
select department ,count(*)"Total Employees",sum(salary)from Employees group by department order by max(salary) DESC;  
-- task 4
